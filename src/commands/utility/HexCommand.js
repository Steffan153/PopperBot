...
module.exports = class CatusCodeCommand extends Command {
      ...
      args: [
        {
          ...
          type: 'string',
          default: 'random',
          prompt: "What hex do you want do show? Or type 'random' to make a random one.",
        },
      ],
    });
  }
  ...
  async run(msg, { s }) {
    if (s !== 'random' && !/^([0-9a-f]{3}){1,2}$/i.test(s)) {
      return msg.say(
        new MessageEmbed().setColor('#FF9AA2').setTitle('Invalid usage. Please put a valid hex code in.')
      );
    }
