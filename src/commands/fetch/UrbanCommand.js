...
module.exports = class UrbanCommand extends Command {
  ...
  async run(msg, { word }) {
    const old = await msg.say(new MessageEmbed().setColor('#92DFF3').setTitle('Hold on there, searching...'));
    urban(word).first((o) => {
      if (!o) {
        return old.edit(new MessageEmbed().setColor('#FF9AA2').setTitle('An error occoured!'));
      }
      old.edit(
        new MessageEmbed()
          .setColor('#B5EAD7')
